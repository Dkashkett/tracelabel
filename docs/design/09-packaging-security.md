...
- **E2E smoke (Playwright):** `tracelabel demo` → j → 1 → type reason → Enter → assert
  annotation row exists and progress incremented. This one test protects the entire pitch.
