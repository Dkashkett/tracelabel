...
from tracelabel import db, export
...
def seeded(conn, tmp_path):
    ...
    db.import_trace(conn, ctf_trace(id="t1"), "jsonl")
    db.import_trace(
        conn,
        ctf_trace(
            id="t2",
...
def test_columns_stable_snapshot(seeded, tmp_path):
    ...
    export.export_annotations(seeded, "task", "csv", joined=False, out=out)
...
def test_joined_turn_level(seeded, tmp_path):
    ...
    export.export_annotations(seeded, "task", "jsonl", joined=True, out=out)
...
def test_joined_trace_level(conn, tmp_path):
    ...
    out = tmp_path / "out.jsonl"
    export.export_annotations(conn, "tracetask", "jsonl", joined=True, out=out)
    rows = [json.loads(line) for line in out.read_text().splitlines()]
    assert len(rows) == 1
    row = rows[0]
...
def test_status_filter(seeded, tmp_path):
    out = tmp_path / "out.jsonl"
    n = export.export_annotations(seeded, "task", "jsonl", joined=False, out=out, status="labeled")
    rows = [json.loads(line) for line in out.read_text().splitlines()]
    assert n == 2
    assert all(r["status"] == "labeled" for r in rows)

    out_all = tmp_path / "out_all.jsonl"
    n_all = export.export_annotations(
        seeded, "task", "jsonl", joined=False, out=out_all, status="all"
    )
...
def test_out_stdout_and_default_name(seeded, tmp_path, monkeypatch, capsys):
    ...
    n = export.export_annotations(seeded, "task", "jsonl", joined=False, out=None)
    assert n == 3
    ...
    n2 = export.export_annotations(seeded, "task", "jsonl", joined=False, out=Path("-"))
...
def test_unknown_task_lists_existing(seeded, tmp_path):
    with pytest.raises(UserError) as ei:
        export.export_annotations(seeded, "nope", "jsonl", joined=False, out=tmp_path / "x.jsonl")
...
def test_suggestions_not_exported(seeded, tmp_path):
    out = tmp_path / "out.jsonl"
    export.export_annotations(seeded, "task", "jsonl", joined=False, out=out)
    rows = [json.loads(line) for line in out.read_text().splitlines()]
